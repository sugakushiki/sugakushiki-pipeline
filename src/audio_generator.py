...
_MISREADING_CATEGORIES: dict[str, list[tuple[str, str]]] = {
    ...
    "math_terms": [
        ...
        ("整数環", "せいすうかん"),
        # 節線 (nodal line, 定在波の節を結ぶ線) — VOICEVOX は 節=ふし と訓読みし
        # フシセン と誤読。正読 せっせん。
        # VOICEVOX 実測 (2026-07-24): 節線→フシセン ❌ / せっせん→セッセン ✅ (in-context も同一)。
        # 節 単独は _AMBIGUOUS_READING_KANJI で要レビュー継続、複合「節線」のみ確定 (安全)。
        ("節線", "せっせん"),
    ],
    ...
    "compounds": [
        ...
        ("居を構え", "きょをかまえ"),
        # 何不自由 (idiom「何不自由なく」) — 何=なに が正だが VOICEVOX は なん と
        # 誤読し ナンフジユウ 化。VOICEVOX 実測 (2026-07-24): 何不自由→ナンフジユウ ❌ /
        # なにふじゆう→ナニフジユウ ✅ (in-context 同一)。固定 idiom のため bare 何 の
        # 文脈依存読み (なに/なん) は壊さない (「何を」「何回」等は非該当)。
        ("何不自由", "なにふじゆう"),
        # 何なのか (疑問「〜は何なのか」) — 何=なん が正だが VOICEVOX は なに と
        # 誤読し ナニナノカ 化。VOICEVOX 実測
        # (2026-07-24): 何なのか→ナニナノカ ❌ / なんなのか→ナンナノカ ✅ (in-context
        # 同一)。exact 4字 phrase-lock、bare 何 は壊さない (「何なの」+「ですか」等は非該当)。
        ("何なのか", "なんなのか"),
    ],
}
