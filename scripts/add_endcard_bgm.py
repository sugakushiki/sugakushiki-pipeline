...
使い方:
  python scripts/add_endcard_bgm.py episodes/017_johann_bernoulli/output_final.mp4 \
      --bgm bgm/angels_dream.mp3 --output episodes/017_johann_bernoulli/output_with_endcard.mp4
...
def main() -> int:
    ...
    p.add_argument("--bgm-start", type=float, default=30.0,
                   help="BGM 内の開始秒 (default: 30.0)")
    ...
    if abs(delta) > 0.5:
        print(f"[WARN] duration delta exceeds 0.5s, check output", file=sys.stderr)
    return 0
