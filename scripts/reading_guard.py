...
残る」箇所 = まさに今回 ある回で user が耳で発見した類 (数=すう, 縁=えん)。

設計意図:
- 断片プローブでは文脈依存誤読 (数+カタカナ -> すう) を取りこぼす。全文
  実測 + 既知リスク辞書で網羅検出する (ある回の学び)。
...
KNOWN_READING_RISKS = [
    ...
    ("自然物", "ブツ", "自然物=しぜんぶつ (物=ぶつ。もの 誤読で シゼンモノ 化。 ある回)"),
    # 割る (除算の動詞): VOICEVOX は終止形「割る」を「わりる」と誤読 (一段活用と誤解析)。
    # ある回ガウスで既知だが global 辞書未登録で ある回「8割る5」再発。audio_generator
