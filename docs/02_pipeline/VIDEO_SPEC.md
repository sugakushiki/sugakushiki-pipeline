...
| 項目 | 値 |
|---|---|
| 尺 | 通常回の枠 (10〜19 分) を超える深掘り回に限り **最大 22 分** まで許容 (`target_duration_minutes` 上限 = 22、`config_validator.py`) |
| 適用 | 1 テーマを通常回より深く扱い、**人物パートを薄くせずに**数学的果実 まで描く場合。量産フォーマット (通常回) の規律を緩めるものではなく、個別判断の例外 |

> 注: 通常回の canonical は上記 **10〜19 分** のまま (このファイル冒頭の SSOT マーカー)。長尺回はこの例外節に従う。`lint_video_spec.py` は canonical (10〜19) の doc 間一致のみを検査するため、本節 (別レンジ) の追記は lint に影響しない。
