...
### かなで語を直したら、直前の助詞に読点を打つ

**語の読みをかなで直すと、その手前の助詞の読みが壊れることがある。** かな化は語の
内部だけの操作ではなく、**隣接する助詞の区切りを変える**。

ある回「その束には端があります」の実測 (合成単位は narration 配列の 1 要素 =
4 文。**1 文だけで probe すると再現しない** ── 単文では壊れた版も ワ と読んだ):

| cloud 文 | 出荷/probe の読み (かな強制 STT 2 回) | 尺 |
|---|---|---|
| `そのたばには端があります` | ニワ **ハジ** (助詞は正しいが 端 が はじ) | 16.7s |
| `そのたばにははしがあります` | **ニハ** ハシ (端は直ったが助詞が壊れた) | 17.8s |
| `そのたばにわはしがあります` | ニワ ハシ | 16.8s |
| **`そのたばには、はしがあります`** | **ニワ ハシ** | **16.5s** |

**採る形は読点** (2026-09-21、user 判断)。`には` の表記を保ったまま境界が明示され、
`わ` 表記のような語中変換の危険も、助詞わ過剰変換 lint との衝突も無い。実測でも
`わ` 版より短く、読点ぶんの間で不自然にはならない。`gen_cloud_readings._cleanup` も
`cloud_tts.strip_for_cloud` もこの形を書き換えないので、パイプラインを素通りする。

- **かなで語を直したら、その直前に助詞 `は` が無いか見る。あれば読点を打つ**
- `cloud_reading_lint` はこの形を警告しない (孤立助詞の型に当てはまらない)。
  **user が通し視聴で耳で見つけた**
- 危険な形は「助詞 `は` の直後にひらがなの語」で、とくに `はは` の並び。
  出荷済み 2,910 行中 6 本しかないので、lint を足すなら雑音は少ない (未実装)
...
| カテゴリ | 内容 |
|---|---|
| 多読み未固定 | 多読み漢字が narration にあり cloud で読みが未固定 (global の SSML 固定語と `episode_config.cloud_reading_overrides` の語は抑止)。読みが文脈で割れる語 (十分=じゅうぶん/じゅっぷん、四つ=よっつ/よつ、正に=せいに/まさに) はどちらかが cloud にあれば固定済み |
| 字母の長音 (エー→エイ) | cloud の単独「エー」(変数 a)。Chirp が長音を引き延ばして a だけ遅くなる。「エーアイ」のような語は対象外 |
...
- **主題名が「ハ」で終わる ep では名前自身の末尾ハに誤発火する**。narration から「ハ終わりカタカナ語」の辞書を抽出し
  (`collect_ha_final_names`)、検出位置の語が辞書名の末尾 (編集距離 1 まで) なら抑止する。
  真陽性「名前 + 誤読助詞ハ」(バナッハハ、) は距離 2 になるため発火が維持される。
  判定は `scan_stt_rules` に一本化され `verify_shipped_audio` と共有 (回帰
)。
