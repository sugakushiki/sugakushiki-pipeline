...
| hook の matcher が主シェルを覆っていない | `qa_quick_guard` は 2026-08-06 まで `matcher: "Bash"` だけで、**このプロジェクトの主シェルである PowerShell ツール経由なら `--qa-quick` が素通り**していた。hook 本体は tool 名を見ずコマンド文字列で判定するので、穴は配線側だけにあった。`edit_sweep_reminder` も `Edit` のみで、`Write` の丸ごと書き直しでは発火しなかった | matcher を `Bash|PowerShell` / `Edit|Write|NotebookEdit` に拡張。 の `REQUIRED_MATCH` が被覆を assert するので、狭めると回帰スイートが落ちる。**新しい実行系ツールが増えたら matcher を見直す** |
