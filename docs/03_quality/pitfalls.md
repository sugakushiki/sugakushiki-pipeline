...
| **「根」が ね と読まれる** | 方程式の根 = こん だが Chirp は ね (植物の根) と読みうる。出荷 wav の STT が intro_02 を「ホウテイシキ ガ モチウル ネ ワ」と書き起こした。**1 文字の漢字は `_POLYPHONE` の表層に登録できない** — 根本/平方根/屋根 のような複合語すべてに一致するため | `cloud_reading_lint._scan_bare_kon` が **前後の文字種**で裸の 根 だけを取り出す (前=漢字/片仮名/長音/数字でない、後=漢字でない)。角 と同じ判定を `_iter_bare_kanji` に共通化。こん/ね は文脈依存なので読みは強制せず明示を促す。出荷 63 ep 全走査で 根 98 件中 37 件が裸、engine=cloud に限れば発火は 1 件。 |
| **`pronunciation_high_risk` に書いた読みが効かない** | あの欄は `audio_generator` が **VOICEVOX のプロンプトヒント**に流すだけで、**engine=cloud では誰も読まない**。ある回は「根 → こん」「正の根 → **せいのこん**(ね と読ませない)」と書いてあったのに 根 が漢字のまま合成テキストに残った。危険は人間が把握し、リポジトリに文字として存在し、そして照合されなかった | `cloud_reading_lint._scan_high_risk_unpinned` が **その語が narration_speech_cloud に元の表記のまま残っていないか**を照合する (narration = 字幕は漢字のままが正しいので見ない)。**全エントリ対象は不可** — 出荷 cloud 20 ep で 1101 件発火し (原論/素数/有理数…= Chirp が普通に正読する硬語)、毎ビルド 50 件の警告は lint を無視する習慣を育てる。**書き手が競合する読みを名指しした語**(`**よみ**` 強調 /「〜と読ませない」/「最重要」) に絞ると **21 件** (約 1 件/ep)。これは件数調整でなく弁別 — 誤読候補まで書いた語こそ多読みハザードで、「原論 → げんろん」は難語メモにすぎない。1 文字表層 (角/数/根) は複合語を巻き込むので専用 scan に委ねる |
...
| 決定論チェックが「collisions 0」と言うのに地図が読めない | 衝突検出は **テキストの矩形どうし**しか比べていなかったので、(a) ラベルが他都市の点の上に乗る (b) 経路の線が文字を貫く の 2 通りが原理的に見えない。ある回は同じ地図で 3 回この型で壊れ、3 回とも preflight は沈黙し、人間の目か Manim Vision QA だけが気づいた。**auto-fix がこれを作る側に回る**のが厄介で、bbox の重なりを消すためにラベルを押し出して隣の都市の点の上に停め、「1 scene(s) auto-fixed and persisted」と報告した | **A: ラベルの所属** = 自分の点までの距離 d1 と最寄りの他都市の点までの距離 d2 が `d2/d1 < 2.0` なら WARN。**距離はラベル矩形の最も近い辺から測る** (中心から測ると `ha="right"` の幅広ラベルが軒並み誤発火する)。**B: 線がラベルを貫く** = 曲線を線分列として矩形に Liang-Barsky クリップし内側 8px 以上で WARN、経路ラベルは自分の曲線を除外。どちらも **advisory (ビルドは止めない)**。較正: route_map 保有 37 ep / 40 scene で A 25 件/10 ep・B 38 件/19 ep、抽出検証した実レンダ 4 件は全て真陽性。 |
| HTML モックが「衝突なし」でも実レンダが壊れる | ある回のレイアウトは `_route_map_layout.html` (1920x1080 モック) で設計し、モックは線の交差を報告していた。しかしモックは往復経路の弓なりを `bow=0.14/-0.22` で近似しており、実際の `generate_route_map` は往復の帰り便に `curve_height = dist * (0.32 + 0.24*k)` = **0.56*dist** を使う。モックの弧は実物よりはるかに浅く、実レンダでは 1580 の点線がタイトルを 189px 貫通していた | モックは配置を詰めるのに有効だが**最終確認は実レンダで取る**。曲線の式を変えたらモック側も合わせる。決定打は出荷物 (`visuals/*.mp4`) で取る |
| ~~`--rebuild-scene` で route_map preflight が走らない~~ → **実際は検査を 1 つも通っていなかった** (2026-08-06 修正) | この行は route_map preflight **1 件だけ**を「将来課題」として挙げていたが、AST で棚卸ししたら `_run_partial_rebuild()` は full build が走らせる**検査 16 系統のうちゼロ**を通っていた。しかも preflight の step 集合が手書きの短縮版 `["assemble","credits","bgm"]` だったため、**必ず音声を合成するのに VOICEVOX 疎通も Cloud API キーも起動時に確認していなかった**。「在庫を 1 シーンだけ直す」という最も慎重にやりたい操作が、最も検査の薄い経路になっていた | 検査を **11 の共有ヘルパー**へ括り出し、**両経路から呼ぶ**。**1 回目の配線は音声と visual の側だけで、画像と字幕の側 4 件（画像 QA Gate 2 / 白縁 / 主題肖像 use_reference gap / フォント被覆）を取りこぼした** ── ken_burns の再ビルドは画像を作り直し、字幕は必ず全編焼き直すのにである。同じ AST 測定をやり直して初めて残りが見えた。コストの高い検査は再ビルドした scene に絞る（STT / Manim Vision QA / 画像 QA の `--scenes`）。決定論で速いもの（白帯・bbox 衝突 = 実測 13.8 秒/17 scene）は絞らない。（100 項目、AST + manim/ken_burns 両方の spy 実行 + 変異）。**教訓 2 つ: 既知の穴を 1 件書くと棚卸ししたつもりになる / 塞いだと言う前に同じ測り方でもう一度測る** |
| STOP ゲートが 1 枚も検査せずに素通りする | `route_map_preflight` は `generate_route_map(preflight_only=True)` が例外を投げた scene を **print 1 行だけ出して `continue`** しており、`unresolved` にも `advisory_out` にも最終行にも残らなかった。呼び出し側から見ると「衝突なし」と区別がつかない ── **全 scene がレンダに失敗しても build は素通りする**。ある回の Gate 2 (「13 scenes checked」と言いながら毎回 3〜5 シーンが未評価だった) と同じ型。上の `--rebuild-scene` を本物のヘルパーで通す検証中に、たまたま実レンダが ImportError を投げて発覚した | 未検査 scene を集めて **名指しで表に出し**、`advisory_out` に `type: preflight_unevaluated` として載せる (pipeline の最終サマリ roll-up に件数が積まれる)。**衝突 (`unresolved`) には混ぜない** — 「壊れている」と「見ていない」は別の事実。 |
| 1 文だけ直したつもりの再ビルドが全文を再合成する | ある回で `--steps audio,...` を選んだため 94 文すべてが再合成され、**Cloud TTS は非決定的なのでテキストが同じでも尺が変わり**、全 23 scene の尺がずれて visual 23 本の再 render（24 分）まで波及し、合計 57 分になった。差は同義語 1 語。**再合成する文数は合成前に 1 秒で出せる**のに誰も見ていなかった | `audio_generator.plan_synthesis()` が合成前に同じキー計算を素通しして hits/misses を数え、`_synthesis_forecast_gate` が予告を出す。**既存キャッシュがあるのに半数以上を再合成する**なら確認を挟む（escape `--allow-full-resynthesis`、初回ビルドと `--force-regen-audio` は対象外）。出荷 63 ep へ当てた実測で発火 1 件（048_khayyam 69 文中 41 文）。**個別 ep を日付から推測して名指しするのをやめ、毎回実測する** |
| auto-bounds の padding 下限が小さい地図を支配し、都市が中央の塊に潰れる | 軸ごとの `max(range * 0.20, 5)` は**平坦な下限**。ある回の 5 都市は経度 4.03 度なのに 14.03 度（データの 3.5 倍）を描き、全ラベルが同じ場所を奪い合った。手作業のレイアウト調整 8 回の真因 | 下限を**地図全体スケール** `max(経度幅, 緯度幅)` の 50% で頭打ちにする（`_auto_bounds_pad`）。**軸ごとに頭打ちにしてはいけない** — 経度が広く緯度が薄い地図（ある回 12.72x3.65 / ある回 10.58x3.18）は緯度の余白が本当に要り、実測で 2 シーンが悪化した。全 31 auto シーンで悪化ゼロ・21 シーンは frame 不変 |
| 部分的な `bounds`（`lat` だけ等）を書くと、書かなかった軸が都市座標を無視して既定値になる | `bounds` が dict として存在した時点で auto 計算が丸ごと skip され、欠けた軸は `bounds.get("lon", _DEFAULT_BOUNDS["lon"])` = 世界既定 `[-85, 45]` に落ちる。ある回が lat のみ指定で経度が既定値のまま出荷されている | 挙動は据え置き（変えると ある回の frame が変わる）だが、**部分 bounds を検出したら WARN を出す**ようにした。両軸書くか、`bounds` ごと省いて auto に任せる |
| 凡例ラベルがそのカテゴリの経路を説明していない（3 回目の再発） | `legend_labels` はカテゴリに**1 つ**のラベルを与えるだけで、そのラベルが全経路について真かを誰も検査していなかった。ある回は career に「官職に就く」を付けたが 4 本中 2 本は家庭教師と単なる転居。生成時の初期案は「留学」「研究」でヴィエトはどちらもしていない。ある回も同型（留学・研究 → 進学・就職に手修正） | 経路ラベルは短い自由文なので文字列一致では判定不能 → **LLM advisory review** (`scripts/check_route_legend.py` / visuals step で自動)。correction は書かせず、該当経路ラベルの逐語引用だけを出させて human が史実で判断 |
| ナレーションが語る土地が地図に無い | ある回は最後の隠棲地としてピレネーを語りながら地図にその点が無く、視聴者は**いま聞いた場所を地図の上に見つけられない**。route_map の検査は衝突・見切れ・所属・凡例色と**すべて配置**の話で、**語られたものが描かれているか**は誰も見ていなかった（逆向きの「画面に出るが語られない年号」は ある回で塞いである） | **LLM advisory review** (`scripts/check_route_places.py` / visuals step で自動)。**辞書照合では駄目**: 全 ep の都市名を辞書にした決定論版を出荷 40 scene で試すと真 1 / 偽 4（団体名「ロンドン数学会」・反実仮想「ベルリンやゲッティンゲンといった大学の職に就く道もあった」・他人の移動「オイラーがサンクトペテルブルクへ」・部分文字列「ル・シャンボン」の「ボン」）。地名が文字列として在るかと、その人がそこに居たかは別問題。**辞書に無い地名を永久に見逃す**のも致命的で、起票の契機であるピレネーは ある回が地図に足すまでどの ep にも無かった。correction は書かせない（正解は「地図に足す」とは限らず「ナレーションから外す」「対象外」もある） |
| 凡例の 2 色が見分けられない | education `#7bc8f6` と career `#4cc9f0` は RGB 距離 47。暗い背景に細い線として描くと同色に見え、**凡例が存在しない区別を名乗る**。誰も色の分離を測っていなかった | education を緑 `#52b788` に変更（距離 106）。加えて `check_route_palette_separation()` が閾値 60（対ペア + 対背景）を smoke test section 21 と render 時に強制。旧配色では出荷済み 15 scene が該当していた |
...
| QA script gate の SourceManager (Sonnet) が 2 回連続 `Could not extract valid JSON from response (length=6953 / 6997)` | 切り詰めではない。nested `claude -p` の transcript (`~/.claude/projects/<proj>/<session>.jsonl`) から生レスポンスを復元すると、どちらも完全な ```json ブロック 1 個 (stop_reason=end_turn) で、唯一の欠陥は長い自由文フィールド `youtube_description_text` の中の**エスケープ漏れの `"`**（`編 "The Foundations of Mathematics" 所収` / `・Cheryl Misak, "Frank Ramsey..." (OUP, 2020)`）。契約自身が `著者名, "書名" (出版年)` と引用符を要求しており、38 個の `\"` の中で 1〜数個を落とすと JSON 全体が死ぬ。しかもこのフィールドを読むコードは存在しなかった。ERROR が `length=` しか出さず生レスポンスも捨てていたため原因調査に transcript 掘りが要った | (1) 契約から `youtube_description_text` を削除し、書名は『』で囲む指示を追加 (`qa_checker._build_source_manager_prompt`)。(2) `claude_backend.extract_json_from_response` を全 ```json ブロック後方優先 + 制御文字許容 + `repair_json_text()`（文脈で終端でないと判る `"` を再エスケープ）に強化、`script_generator.extract_json` も最終手段として同じ repair を使用。(3) parse 失敗時は `episodes/XXX/_qa_<agent>_raw_response.txt` に生レスポンスを残し、ValueError にエラー位置と前後 80 字を含める。 |
...
| Python hook / script の stdout 出力で UTF-8 日本語が Windows cp932 console で文字化け | hook の `print()` が UTF-8 文字列 (日本語・記号) を含む時、Windows console が cp932 で受け取り mojibake (例: `��|�[�g`) になる。過去のケースで hook スクリプトの reminder 出力から発覚。emoji / em-dash と違い日本語はクラッシュせず黙って化けるため発見が遅れる | main() 冒頭で `sys.stdout.reconfigure(encoding="utf-8")` + `sys.stdin.reconfigure(encoding="utf-8")` を呼び出す (Python 3.7+)。新 hook / 新 script で日本語や特殊文字を出力する場合はテンプレ化推奨 |
...
> 構造強化 3 件は「学びを構造化する」cycle の継続。subtitle 日付 Arabic 化 (`dates_to_arabic`) は全エピソード自動適用の構造対応であり、ある回固有の patch ではない。
...
| 落とし穴 | 原因 | 構造化した防御 |
|---|---|---|
| **`episode_config.voicevox_dictionary_additions` に読みを書いても無効 (no-op)** | このフィールドを読むコードが src/・scripts/ に存在しない (廃止済、`voicevox_dict.json` に統合)。ある回で 49 語入れたが全て無視され固有名詞が VOICEVOX デフォルト読みに | `config_validator` に WARN 追加 (populated なら「廃止フィールド・no-op、src/voicevox_dict.json に登録せよ」)。smoke section 2 で表面化 |
...
| **narration 編集→再ビルドで字幕(旧)/音声(新) が desync** | `--steps` に subtitles を入れ忘れ (visuals,assemble のみ)→字幕.srt が旧 narration 由来。従来は assemble 完走後の G2 検査で検出=20分の無駄ビルド | `pipeline._subtitle_staleness_check` で assemble 直前に fail-fast (stale-visual preflight の字幕版、`_subtitles_meta.json` の narration_hash と現 scene_def を照合、`--allow-stale-subtitles` opt-out)。**narration 編集時は subtitles と visuals を両方 steps に含める** |
...
| 問題 | 原因 | 対策 |
|---|---|---|
| **字幕が声より最大 1.8 秒早く切り替わる** | 1 要素に複数文が入るとき文境界を推定していたが、その予測が**壁時計時間をモーラ比で割っていた**。無音は語数に比例せず**読点に集まる**ので、読点の多い文は自分のモーラ比より長い壁時計時間を要し、以降の境界が手前にずれる。math_01 では 1.2 秒ずれ、真の文末が許容窓 (±1.1s) の外に出て**読点の間**が無競争で文末に採用された | `sentence_align.speech_time_edges()` = モーラ比を**発話時間**に配分し実測無音を歩いて壁時計に戻す。文の内側のキュー配分にも同じ補正 (`subtitle_generator.distribute_measured`)。**正解の作り方 = 境界の前後を切って STT にかけると前の文の末尾か次の文の頭かで一意に決まる** |
| **数字の多い文だけ境界を外す** | モーラ推定が数字を一律「1 桁 2 モーラ」で数えていた。実際は 120=ひゃくにじゅう=5 (桁あたり1.67) / 39=6 (3.0) / 1947=12 (3.0) と倍近く違い、数字だらけの文では誤差が打ち消し合わない | `sentence_align.numeral_morae()` で数詞として読み下す。STT で確定した 14 箇所の平均誤差 0.48 秒 → **0.003 秒** |
| **修正済みの動画をレビューしてもらえない** | ビルド後 `temp_videos/` へコピーするのを忘れ、**user は修正前の版を見て、直っている 4 件を「なおっていない」と再指摘**。レビュー 1 周が無駄になった。検出する仕組み (`post_build_verify` check 8) は ある時点 からあったが **pipeline から呼ばれておらず**、強制は memory の「必ず実行」という記述だけだった | pipeline 末尾で `post_build_verify` を自動実行 (11 検査) + コピーも pipeline が行う (`--no-temp-video-copy`)。**検査を足したら同じ変更で配線し、回帰テストに「呼ばれていること」を入れる** |
| **配線しても出力が読者に届かない** | 上記を配線した直後、pipeline は `[OK] 11. 画像の下隅シート` とだけ出し**シートのパスを出さなかった** ([ACTION] を出す処理が CLI の `main()` にしか無い)。見出しも「(9 checks)」と数字を焼き込んだまま | 出力を `format_actions()` に切り出して CLI と pipeline で共用。件数は `len(checks)` から出す。**「走る」と「読まれる」は別** |
| **章タイムスタンプが古いまま出荷しかける** | description.txt の章時刻は timing.json 由来だが、書いた後は誰も照合しない。timing だけ変えて `credits` を飛ばすと 1:38 のまま残る。既存の鮮度検査 (check 3) は **mtime 比較で、しかも timing.json を見ていない** (加えて mtime 方式は出荷済み 61 本中 42 本が STALE 判定=ほぼノイズ) | `post_build_verify` check 10 で **値を再計算して照合**。出荷 61 本で 52 本一致・8 本は全章一律 1 秒差 (intro_pause の世代差)・**012_descartes だけ本物の食い違い (要確認)** |
| **生成画像の隅に署名が描き込まれる (13 枚中 6 枚)** | 画像モデルが油絵の署名を描く。白縁 lint は白い縁しか見ない。1 枚ずつ見ていたので 2 枚で sweep が止まり、**残り 4 枚を見逃した (1 枚はエンドカードで 10 秒表示)** | `post_build_verify` check 11 = **全画像の下隅を 1 枚に並べたシート**(ガンマを持ち上げる)。**検出器は 2 回作って 2 回とも失敗** ── ①高周波の閾値は本の背と机の縁を提案し、その上に作った修復が草原に机を貼った ②「平らな面の上の細い構造」の順位付けは既知 6 個のうち上位 12 位に 2 個しか入らず目を誤った場所へ誘導した |
| **署名を消そうとして画像を壊す** | 近傍の矩形をコピーする方式は**内容を動かす**ので、草原に机が貼り込まれた。局所メディアンとの差が大きい画素を全置換する方式は文字は消えるが**テクスチャごと平滑化**して四角い滲みが残る | **ストロークの画素だけ選んで拡散補間**。ただし条件は画像ごとに違う (筆致が地より明るい絵では「暗い方」条件で捕まらない / 箱の中に本物の暗い構造があるとそれを消す)。**箱は検出器に決めさせず目盛り付き 3 倍拡大で読む**。閾値は面積比 (箱の 8-9%)。薄い痕は**消さない判断**もする (person_07/math_04 は周囲に構造があり、消すと滲みのほうが目立つ) |
| **企画で決めた語が本編に書かれないまま完成** | 双対性の説明で『ミニマックス』を一行だけ入れると決めたのに、どの scene にも書かれず**完成した動画を見た user の指摘で判明**。決めたことが計画メモにしかなく照合する仕組みが無かった | `episode_config.json` の `required_phrases` + smoke test section 18b。照合先は **narration のみ** |

> ある時点 の学び: **「原理的に無理」と言う前に自分の式を疑う** ── 1 箇所だけ外れるのを「候補が等距離の賭けだから」と説明したが、実際は別のモデル誤り (数詞のモーラ) で、直したら 14/14 になった。**絶対時刻を焼き込んだテストは合成をやり直すと壊れる** ── 14 箇所の STT 正解を assert に書いたら次のビルドで最大 0.810 秒ずれて落ちた。アルゴリズムは合成データで固定し、実音声側は**やり直しても成り立つ性質**だけを見る。**変異テストの変異自体を疑う** ── 「`run_all(` が pipeline にある」を部分文字列で検査していたため `xrun_all(` への変異が素通りした (AST で Call の attr を見る)。
