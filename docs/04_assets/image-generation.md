...
### `use_reference: false` と `is_subject: false` — 似ているが別物

主題者以外の人物を描くシーン（例: ライプニッツ回の Newton）では **2 つの別々の問題**が
起きる。フラグも 2 つあり、**どちらか一方では足りない場合がある**。

| フラグ | 既定 | 何を止めるか | 効く側 |
|---|---|---|---|
| `use_reference: false` | true | 主題者の参照写真が別人のシーンに使われる（リファレンス汚染） | **生成**側 |
| `is_subject: false` | true | そのシーンを主題者の cross-scene 外見一貫性チェックの対象から外す | **検証**側 |

一貫性 QA は intro / person シーンを「同じ人物」の前提で見るので、脇役を混ぜたままだと
「同一人物なのに外見が不統一」という**偽陽性**が出る。逆に、主題者に写真が無い古代回では
全シーンが実質 `use_reference: false` になるため、**脇役の判別は `is_subject` でしかできない**。

**主題者のシーンにはどちらも付けない。** 上記「実写リファレンスが効く条件」のとおり
未設定が既定 `true` なので、主題者の肖像には何も書かないのが正しい。多人数シーンは
参照を効かせるより text-only のほうが安定する（眼鏡・髭などの識別的特徴は
`source_prompt` に明記して補う）。

なお「人物がいない」シーン（静物・風景）は `no_human: true` で、これは
`use_reference` を強制的に false にする。
